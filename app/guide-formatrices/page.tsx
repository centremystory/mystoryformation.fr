...
const CAT_LABEL: Record<string, string> = {
  accueil: "Accueil & positionnement",
  animation: "Animation du cours",
  evaluation: "Évaluation (initiale / finale)",
  emargement: "Émargement & suivi",
  conformite: "Conformité pédagogique",
  autre: "Autre",
};
const CAT_ORDRE = ["accueil", "animation", "evaluation", "emargement", "conformite", "autre"];
...
export default function GuideFormatricesPage() {
  ...
  const charger = useCallback(async () => {
    ...
    const [r, me] = await Promise.all([
      fetch("/api/guide-formatrices", { cache: "no-store" }),
  ...
  async function sauverEdit(id: string) {
    ...
    await fetch("/api/guide-formatrices", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, ...e }) });
    setEdit((p) => { const n = { ...p }; delete n[id]; return n; });
    await charger();
  }
  async function archiver(id: string) {
    if (!window.confirm("Archiver cette fiche ?")) return;
    await fetch("/api/guide-formatrices", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, action: "archiver" }) });
    await charger();
  }
  async function ajouter() {
    if (!nouv || !nouv.titre.trim() || !nouv.contenu.trim()) return;
    await fetch("/api/guide-formatrices", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(nouv) });
    ...
    <>
      ...
      <div className="mx-auto max-w-3xl px-4 py-6">
        <div className="page-header">
          <h1 className="page-title">Guide Formatrices</h1>
          <p className="page-subtitle">Le guide pédagogique MYSTORY : accueillir, animer, évaluer, émarger — proprement. À enrichir au fil du temps.</p>
        </div>

        <div className="card p-3 mb-4 flex items-center gap-2">
          <input value={recherche} onChange={(e) => setRecherche(e.target.value)} placeholder="Rechercher une fiche…" className="input flex-1" />
          {peutEditer && !nouv && (
            <button onClick={() => setNouv({ categorie: "accueil", titre: "", contenu: "" })} className="btn-primary whitespace-nowrap">+ Fiche</button>
          )}
        </div>
        ...
        {peutEditer && nouv && (
          <div className="card p-3 mb-4 space-y-2">
            ...
            <input value={nouv.titre} onChange={(e) => setNouv({ ...nouv, titre: e.target.value })} placeholder="Titre de la fiche" className="input w-full" />
        ...
        {!charge && fiches.length === 0 && (
          <div className="card"><div className="empty-state"><p className="text-sm text-gray-500">Aucune fiche pour l&apos;instant. {peutEditer ? "Ajoutez la première avec « + Fiche »." : ""}</p></div></div>
        )}
