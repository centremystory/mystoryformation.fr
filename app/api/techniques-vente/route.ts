/**
 * MYSTORY — /api/techniques-vente  (guide commercial interne, éditable)
...
const CATEGORIES = ["decouverte", "formation", "examen", "closing", "conformite", "autre"];
...
export async function GET(req: NextRequest) {
  ...
  let q = supabaseAdmin.from("techniques_vente")
...
export async function POST(req: NextRequest) {
  ...
  const { data: max } = await supabaseAdmin.from("techniques_vente")
  ...
  const { data, error } = await supabaseAdmin.from("techniques_vente")
    .insert({ slug, categorie, titre, contenu, ordre, auteur: u.email ?? null })
    .select("id, slug, categorie, titre, contenu, ordre").single();
  if (error) return NextResponse.json({ ok: false, erreur: error.message }, { status: 500 });
  await journal("technique_vente", (data as any).id, "creee", { titre }, u.email ?? null);
...
export async function PATCH(req: NextRequest) {
  ...
  if (b?.action === "archiver") {
    const { error } = await supabaseAdmin.from("techniques_vente").update({ actif: false }).eq("id", id);
    if (error) return NextResponse.json({ ok: false, erreur: error.message }, { status: 500 });
    await journal("technique_vente", id, "archivee", {}, u.email ?? null);
  ...
  const { error } = await supabaseAdmin.from("techniques_vente").update(maj).eq("id", id);
  if (error) return NextResponse.json({ ok: false, erreur: error.message }, { status: 500 });
  await journal("technique_vente", id, "modifiee", {}, u.email ?? null);
  return NextResponse.json({ ok: true });
}
