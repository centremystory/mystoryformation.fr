/**
 * MYSTORY — /api/documents/test
 * Génère la pièce « Test de positionnement / Test final » (corrigé détaillé) du dossier à partir
 * du test réellement passé (moteur d'évaluations) : QCM question par question avec la correction.
 * POST { dossierId, phase: "initial" | "final" }
 */
import { NextRequest, NextResponse } from "next/server";
import { requireUser, UnauthorizedError } from "@/lib/auth";
import { genererDocTest } from "@/lib/testDoc";
...
export async function POST(req: NextRequest) {
  ...
  const res = await genererDocTest(dossierId, phase as "initial" | "final", email);
  if (!res.ok) return NextResponse.json({ ok: false, erreur: res.raison ?? "Génération impossible." }, { status: 409 });

  const piece = phase === "final" ? "test_final" : "test_positionnement";
